...
class FundRequestPaymentHistoryFragment : Fragment(), ProgressListener, AllPaymentHistoryAdapter.ItemClick {
    ...
    private fun getRequestPaymentHistory() {
        ...
        compositeDisposable.add(
                requestInterface.getPaymentHistory(requestBody).observeOn(AndroidSchedulers.mainThread())
                        ...
                        .subscribe({
                            ...
                            payment_history_list.adapter = RequestPaymentHistoryAdapter(it?.body()?.data, this)
    ...
    fun onSearch(filterStartDate: String, filterEndDate: String) {
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance(sdate: String, edate: String, familyId: String?, userId: String?): FundRequestPaymentHistoryFragment {
            return FundRequestPaymentHistoryFragment().apply {
                arguments = Bundle().apply {
                    putString("SDATE", sdate)
                    putString("EDATE", edate)
                    putString(Constants.Bundle.FAMILY_ID, familyId)
                    putString(Constants.Bundle.ID, userId)
                }
            }
        }
    }
